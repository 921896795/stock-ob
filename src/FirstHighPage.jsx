...
const columns = [
  ...
  { title: '首次新高日期', dataIndex: 'first_date', key: 'first_date', width: 130, sorter: true },
]

export default function FirstHighPage() {
  ...
  useEffect(() => {
    fetch('/api/firsthigh/first-dates')
      .then(r => r.json())
      .then(setFirstDates)
      .catch(() => {})
  }, [])
  ...
  const fetchData = useCallback(async (p = 1) => {
    ...
    try {
      ...
      const res = await fetch(`/api/firsthigh/stocks?${params}`)
  ...
  }, [date, prefix, period, firstDate, keyword])
  ...
  return (
    <>
      ...
      <div className="filter-bar">
        ...
        />
        <Select
          allowClear placeholder="首次新高日期" style={{ width: 160 }}
          value={firstDate} onChange={(v) => setFirstDate(v)}
          options={firstDates.map(d => ({ label: d, value: d }))}
          showSearch filterOption={(input, option) => (option?.label ?? '').includes(input)}
        />
        <Select
